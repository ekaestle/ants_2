...
from ants_2.tools.correlations import my_centered
import numpy as np
...
class CorrTrace(object):
    ...
    def __init__(self,cha1,cha2,sampling_rate,corr_type='ccc',
        ...
        if isinstance(self.stck_int, int):
            if self.stck_int > 0:
...
#        del corr
    ...
    def _add_corr_fd(self,corr,t,output_format):
        
        """
        Add one correlation window in the freq domain to the stack
        """
              
        if len(self.stack_fd) == 0:
            self.stack_fd = corr
        else:
            self.stack_fd += corr
        ...
        if self.stck_int is not None:
            ...
            if isinstance(self.stck_int, int):
                if self.cnt_int == self.stck_int and self.stck_int > 0:
                    # write intermediate result
                    self._ifft_fd_int_stack()
                    self.write_int(t)
                    self.cnt_int = 0
                    self.pstak = None

            elif self.stck_int == 'monthly':
    ...
    def _ifft_fd_int_stack(self):
        
        ccv = np.fft.irfft(self.pstak)/self.cnt_int
    ...
    def _ifft_fd_stack(self):
        
        ccv = np.fft.irfft(self.stack_fd)/self.cnt_tot
        # I think using ifft is not necessary even though the time fdomain signal may not be purely real anymore
        #ccv_neg = np.conjugate(ccv)[::-1]
        #ccv = np.concatenate((ccv,ccv_neg[1:-1]))                    
        #ccv = np.real(np.fft.ifft(ccv))
        ccv = np.fft.ifftshift(ccv)
    
        self.stack.data = ccv#my_centered(ccv,2*max_lag_samples+1)
        # cutting the lag time deteriorates the freq-domain signal significantly
        # not recommended for Bessel-function methods such as phase-vel picking
    ...
    def write_monthly(self):
        ...
        filename = os.path.join('data','correlations','{}.{}.{}.SAC'.format(self.id,self.year,self.month))

        tr = Trace()
        tr.data = self.pstak
        tr.stats = self.stack.stats
